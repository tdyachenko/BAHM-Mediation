...
{ 
  ...
  # ncol= nvar, including lambdas and sigmas
  Mpsrf = matrix(0,nrow=nrow(table01),ncol=1)
  for(j in 1:nrow(table01)) 
  { index1 = c(which(table01[j,]==1))   # select seeds that are grouped together, indexed 1 in table01
  ...
  # could not do parallel, mcmc.list is not working with parallel
  
  DIC = FUN_DIC_mediation(inputdata, datafile,burnin,ModelFlag=2)  #### ADDED, needs to be tested

  temp = which(Mpsrf<RhatCutoff)   # pick elements of the vector where solutions are separated into "good" groups
